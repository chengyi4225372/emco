...
function initCustomMap() {
	...
	jQuery('.map-holder, .map-area').each(function() {
		var mapHolder = jQuery(this);
		var map = mapHolder.find('.map');
		mapHolder.data('CustomMap', new CustomMap({
			holder: this,
			map: '.map',
			startCooords: map.data('center'),
			defaultOptions: {
				maxZoom: 30,
			...
			onMarkersAdded: function(self) {
				var timer;
				if(self.addedMarker.length === 1) {
					google.maps.event.addListenerOnce(self.mapCanvas, 'bounds_changed', function(event) {
						self.mapCanvas.setZoom(self.options.defaultOptions.zoom);
					});
...
function initValidation() {
	...
	jQuery('form.validate-form').each(function(){
		...
		function checkField(i, obj) {
			...
			var currentParent = currentObject.closest('.required-field');
...
function initOpenClose() {
	jQuery('.categories-form').openClose({
...
CustomMap.prototype = {
	...
	createMap: function() {
		...
		this.getMapStyles(this.map.attr('data-styles'), function(data) {
			usRoadMapType = new google.maps.StyledMapType(data, {name: 'Custom Map'});
	...
	getMapStyles: function(url, complete) {
	...
	prepareMarkers: function(data) {
		...
		jQuery.each(data.markers, function(key, val) {
			...
			if (val.icon && typeof val.icon === 'object') {
				var iconData = {};
				if (val.icon.url && val.icon.url.length) {
					iconData.url = val.icon.url;
				}
				if (val.icon.size && val.icon.size.length) {
					iconData.size = new google.maps.Size(val.icon.size[0], val.icon.size[1]);
				}
				if (val.icon.anchor && val.icon.anchor.length) {
					iconData.anchor = new google.maps.Point(val.icon.anchor[0], val.icon.anchor[1]);
				}
				newMarker.icon = iconData;
			}
	...
	addMarkers: function() {
		...
		setTimeout(function() {
			...
			self.makeCallback('onMarkersAdded', self);
		}, 100);
	},
	...
	getDirrectionLink: function(coordinates) {
		return 'https://maps.google.com/maps?daddr=' + coordinates.G + ','  + coordinates.K;
	}
};
		...
		attachEvents: function() {
			...
			self.outsideClickHandler = function(e) {
				if (self.options.hideOnClickOutside) {
					var target = $(e.target);
					if (!target.is(self.holder) && !target.closest(self.holder).length  && !target.hasClass('jcf-option')) {
						self.hideSlide();
					}
				}
			};
	...
	MobileNav.prototype = {
		...
		show: function() {
            if (this.isFirstTime) {
                ...
                szContactFinderTriggerInitFlyout();
            }
...
;(function($, window) {
	...
	FixedScrollBlock.prototype = {
		...
		destroy: function() {
            ...
            $('body').removeClass('stickeyHeader');
			this.win.off({
				resize: this.onResize,
				scroll: this.onScroll
			});
