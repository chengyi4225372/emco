...
class Clearimage extends BasicAdmin {

    private $dataform = 'clear_image';
    ...
    protected function _data_filter(&$data) {
        foreach ($data as $key => $val) {
            if($val['cid'] != 0){
                $data[$key]['clear_titles'] = Db::name('clear_t')->where('id', '=', $val['cid'])->value('title');
            }
        }
    }
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/Clearimage/index')];
