...
class Categorys extends BasicAdmin {

    private $dataform = 'category';
...
//            $db->whereBetween('create_at', [$start_time, $end_time]);
    ...
    protected function _data_filter(&$data) {
        foreach ($data as $key => $val) {
                $data[$key]['category'] = Db::name('list')->where('id', '=', $val['lid'])->value('name');
        }
    }
    /**
     * 添加
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/categorys/index')];
