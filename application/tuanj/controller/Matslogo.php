...
class Matslogo extends BasicAdmin {

    private $dataform = 'mats_logo';


    public function index() {
        $this->title = '入口垫二级类别图标';
    ...
    protected function _data_filter(&$data) {
        foreach ($data as $key => $val) {
            $data[$key]['mats_logo'] = Db::name('mats_two')->where('id', '=', $val['pid'])->value('title');
        }
    }
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/matslogo/index')];
